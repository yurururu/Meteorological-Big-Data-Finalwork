...
def run_training(args, data):
    ...
    for epoch in range(args.epochs):
        ...
        if is_gan(args):
            discr_loss = running_discr_loss/len(data)
            print('Epoch {}, Train Loss: {:.5f}, Discr. Loss{:.5f},time:{:.5f} s'.format(
                epoch+1, loss, discr_loss,end-start))
        else:
            print('Epoch {}, Train Loss: {:.5f},time:{:.5f} s'.format(epoch+1, loss, end-start))
        start = time.time()
        if is_gan(args):
        ...
        print('Val loss: {:.5f},time:{:.5f} s'.format(val_loss,end-start))
...
def evaluate_model(data, args):
    ...
    print("测试集评估花费时间：{:.5f} s".format(end-start))
    if is_gan(args):
    ...
    print("测试集评估指标计算花费时间：{:.5f} s".format(end-start))
...
def calculate_scores(args):
    ...
    l2_crit = nn.MSELoss().to(device)#均方误差MSE
    l1_crit = nn.L1Loss().to(device)#平均绝对误差MAE
    ...
    for i,(lr, hr) in enumerate(val_data):
        ...
        # multiscale_structural_similarity_index_measure多尺度结构相似性指数（MS-SSIM):计算预测图像和真实图像之间的多尺度结构相似性指数，并累加到 ms_ssim 变量中。
